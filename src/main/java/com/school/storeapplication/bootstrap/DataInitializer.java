    ...
    @Bean CommandLineRunner seed(CategoryRepository cats, ProductRepository products, UserRepository users, PasswordEncoder pe) {
        return args -> {
            if (cats.count() == 0) {
                ...
                products.save(Product.builder().name("Classic T-Shirt").description("100% cotton")
                        .price(new BigDecimal("19.99")).sku("TSHIRT-001").stock(100)
                        .imageUrl("https://picsum.photos/200/300").category(men).active(true).build());
                products.save(Product.builder().name("Denim Jacket").description("Slim fit")
                        .price(new BigDecimal("59.99")).sku("JACKET-001").stock(50)
                        .imageUrl("https://picsum.photos/200/301").category(women).active(true).build());
            }
            if (users.count() == 0) {
                users.save(User.builder().email("buyer@example.com").password(pe.encode("password"))
                        .firstName("Buyer").lastName("User").roles(Set.of(Role.ROLE_BUYER)).build());
            }
            if (users.findByEmail("admin@store.com").isEmpty()) {
                var admin = User.builder()
                        .email("admin@store.com").password(pe.encode("Admin#123"))
                        .roles(Set.of(Role.ROLE_ADMIN)).build();
                users.save(admin);
            }
            if (users.findByEmail("seller@example.com").isEmpty()) {
                var seller = User.builder()
                        .email("seller@example.com").password(pe.encode("Seller#123"))
                        .firstName("Seller").lastName("User")
                        .roles(Set.of(Role.ROLE_SELLER)).build();
                users.save(seller);
            }
        };
    }
